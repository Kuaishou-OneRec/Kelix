...
def load_safe_tensors(path, max_workers=None):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # 使用 tqdm 显示进度
        for future in tqdm(futures, desc="加载 safetensors 文件", total=len(file_paths)):
            # 合并结果
...
model1 = load_safe_tensors(model_name1)
model2 = load_safe_tensors(model_name2)



import torch

def compare_model_parameters(params1, params2, threshold=1e-6):
    """
    比较两个模型的参数，找出不同的权重
