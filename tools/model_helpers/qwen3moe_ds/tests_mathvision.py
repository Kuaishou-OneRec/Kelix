...
mvtext={
    1:"Which number should be written in place of the question mark?",
    53:"Max has 10 dice. Which one of the following solids can he build with them?",
    88:"There are 12 children in front of a zoo. Susi is the 7th from the front and Kim the $2 \mathrm{nd}$ from the back. How many children are there between Susi and Kim?"
}


def make_inputs(a,b, _type="image", with_sys=True, index=1):
...
"""  if with_sys else "You are a helpful assistant."
} ,
        {
            "role": "user",
            "content": [
                {"type": "image", "image": f"/llm_reco/lingzhixin/recovlm_qw0510/recovlm/tools/model_helpers/qwen3moe_ds/mv{index}.jpg"},
                {"type": "text", "text": mvtext[index]},
            ],
        }
    ]
    # if not with_sys:
...
for MODEL_DIR in [
        ...
        "/mmu_mllm_hdd_2/zhouyang12/output1/Keye/0.8.6/Stage3/8b/v16/step2400/global_step2400/converted/",
        "/mmu_mllm_hdd_2/zhouyang12/output1/Keye/0.8.6/Stage3/8b/v17/step2400/global_step2400/converted/"
    ]:
    print(f"\n\nMODEL_DIR={MODEL_DIR}")
    ...
    for index in [1,53,88]:
        for with_sys in [True, False]:
            print(generate_ascii_banner(f"testing " + str(index) +  f"with_sys={with_sys} ..." ))
            messages, inputs, messages_old = make_inputs(100,100, with_sys=with_sys, index=index)
