...
class Qwen3_VLVisionPatchEmbed(nn.Module):
    def __init__(
        self,
        patch_size: int = 14,
...
class Qwen3_VLVisionRotaryEmbedding(nn.Module):
...
class Qwen3_VLVisionTransformerPretrainedModel(Qwen3_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.rotary_pos_emb = Qwen3_VLVisionRotaryEmbedding(head_dim // 2)
...
class Qwen3_VLRotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen3_VLConfig, device=None):
        ...
        if config is None:
            logger.warning_once(
                "`Qwen3_VLRotaryEmbedding` can now be fully parameterized by passing the model config through the "
...
class Qwen3_VLSdpaAttention(Qwen3_VLAttention):
    """
    Qwen3_VL attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `Qwen3_VLAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """
...
class Qwen3_VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen3_VLConfig, layer_idx: int):
        ...
        self.self_attn = QWEN3_VL_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)
...
class Qwen3_VLForConditionalGeneration(Qwen3_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual = Qwen3_VLVisionTransformerPretrainedModel._from_config(config.vision_config)
...
class Qwen3_VLForConditionalGeneration_moonvit(Qwen3_VLPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    config_class = Qwen3_VLConfig
    _no_split_modules = ["Qwen3_VLDecoderLayer", "MoonVitEncoderLayer"]
...
class Qwen3_VLForConditionalGeneration_siglip(Qwen3_VLPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    config_class = Qwen3_VLConfig
    _no_split_modules = ["Qwen3_VLDecoderLayer", "SiglipEncoderLayer"]
