...
class Qwen2VLImageProcessor_moonvit(BaseImageProcessor):
    r"""
    Constructs a Qwen2-VL image processor that dynamically resizes images based on the original images.
    ...
    def patchify(self, image: torch.Tensor) -> tuple[torch.Tensor, list[int, int]]:
        ...
        grid_hw = (1,H // patch_size, W // patch_size)
    ...
    def preprocess(
        ...
        if images is not None:
            ...
            for image in images:
                ...
                pixel_values.extend(patches)
                vision_grid_thws.extend(image_grid_thw)
            pixel_values = torch.concat(pixel_values, dim=0)
            #vision_grid_thws is list of numpy array
            vision_grid_thws = np.array(vision_grid_thws)
            data = {"pixel_values": pixel_values, "image_grid_thw": vision_grid_thws}
        ...
        if videos is not None:
            ...
            for images in videos:
                ...
                pixel_values.extend(patches)
                vision_grid_thws.extend(video_grid_thw)
            pixel_values = torch.concat(pixel_values, dim=0)
            #vision_grid_thws is list of numpy array
            vision_grid_thws = np.array(vision_grid_thws)
            data = {"pixel_values_videos": pixel_values, "video_grid_thw": vision_grid_thws}
...
class Qwen2VLImageProcessor_siglip(BaseImageProcessor):
    r"""
    Constructs a Qwen2-VL image processor that dynamically resizes images based on the original images.
    ...
    def _preprocess(
        ...
        print(init_patches.shape, patches.shape, (grid_t, grid_h, grid_w))
