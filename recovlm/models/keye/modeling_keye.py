...
class KeyeVisionFlashAttention2(nn.Module):
...
class KeyeRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            print(inv_freq_expanded.float().shape, position_ids_expanded.float().shape, 9889457444)
            print(inv_freq_expanded.float().dtype, position_ids_expanded.float().dtype, 9889457444)
            print(inv_freq_expanded.float().device, position_ids_expanded.float().device, 9889457444)
