...
from .configuration_keye_qmoe import KeyeQMoeConfig, KeyeQMoeVisionConfig
...
def get_transformers_version():
    try:
        return pkg_resources.get_distribution("transformers").version
        ...
        try:
            ...
            return "0.0.0"  # 默认返回最低版本

def is_version_gte(current, target):
    """检查当前版本是否大于或等于目标版本"""
...
USE_NEW_TRANSFORMERS = is_version_gte(transformers_version, "4.52.0")

# 根据版本动态导入
if USE_NEW_TRANSFORMERS:
    print(f"使用 transformers {transformers_version} 的新导入方式")
    from transformers.integrations import use_kernel_forward_from_hub
    from transformers.modeling_flash_attention_utils import FlashAttentionKwargs
else:
    print(f"使用 transformers {transformers_version} 的兼容导入方式")
    
    # 模拟的装饰器
    def use_kernel_forward_from_hub(kernel_type=None):
        def decorator(cls):
            setattr(cls, "_kernel_type", kernel_type)
            setattr(cls, "_is_fake_kernel", True)
            return cls
        return decorator
...
_CONFIG_FOR_DOC = "KeyeQMoeConfig"
...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: KeyeQMoeVisionConfig):
        super().__init__()
        self.config = config
...
class SiglipAttention(nn.Module):
    ...
    def forward(
        ...
        use_flash_attn = (cu_seqlens is not None)
        ...
        if not use_flash_attn:
            ...
            if self.config._attn_implementation != "eager":
                if self.config._attn_implementation == "sdpa" and output_attentions:
                    ...
                    raise ValueError(f"{self.config._attn_implementation} is not a valid attention implementation")
...
class SiglipEncoderLayer(nn.Module):
    def __init__(self, config: Union[KeyeQMoeVisionConfig]):
        super().__init__()
        self.embed_dim = config.hidden_size
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            width = (
                ...
                if isinstance(self.config, KeyeQMoeConfig)
...
SIGLIP_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`KeyeQMoeConfig`]): Model configuration class with all the parameters of the model.
...
class SiglipEncoder(nn.Module):
    ...
    Args:
        config: KeyeQMoeConfig
    """

    def __init__(self, config: KeyeQMoeConfig):
        super().__init__()
        self.config = config
...
class SiglipVisionTransformer(nn.Module):
    ...
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=KeyeQMoeVisionConfig)
    def forward(
        self,
        pixel_values,
        output_attentions: Optional[bool] = None,
...
class SiglipVisionModel(SiglipPreTrainedModel):
    config_class = KeyeQMoeVisionConfig
...
class KeyeQMoeAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: KeyeQMoeConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = KeyeQMoeRotaryEmbedding(config=config)
...
class KeyeFlashAttention2(KeyeQMoeAttention):
    """
    Keye flash attention module, following Keye attention module. This module inherits from `KeyeQMoeAttention`
...
class KeyeQMoeSdpaAttention(KeyeQMoeAttention):
    """
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "KeyeModel is using KeyeQMoeSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
KEYEQMOE_ATTENTION_CLASSES = {
    "eager": KeyeQMoeAttention,
    "flash_attention_2": KeyeFlashAttention2,
    "sdpa": KeyeQMoeSdpaAttention,
}
...
class KeyeVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        self.attn = KEYEQMOE_ATTENTION_CLASSES[attn_implementation](
...
#             print(f"aaaaaa", module.weight.dtype, module.weight.device, count)
#             if module.bias is not None:
#                 module.bias.data.zero_()
#         elif isinstance(module, nn.Embedding):
#             module.weight.data.normal_(mean=0.0, std=std)
#             print(f"bbbbb", module.weight.dtype, module.weight.device, count)
#             if module.padding_idx is not None:
#                 module.weight.data[module.padding_idx].zero_()
#         elif isinstance(module, Qwen3MoeRMSNorm):
#             module.weight.data.fill_(1.0)
#             print(f"ccccc", module.weight.dtype, module.weight.device, count)
...
#             print(111111, module.weight.device)
#             module.weight.data.normal_(mean=0.0, std=std)
#             if module.bias is not None:
#                 module.bias.data.zero_()
#         elif isinstance(module, nn.Embedding):
#             print(222222, module.weight.device)
#             module.weight.data.normal_(mean=0.0, std=std)
#             if module.padding_idx is not None:
#                 module.weight.data[module.padding_idx].zero_()
...
class KeyeQMoeAttention(nn.Module):
    ...
    def __init__(self, config: KeyeQMoeConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        self.q_norm = KeyeQMoeRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = KeyeQMoeRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class KeyeQMoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.experts = nn.ModuleList(
            [KeyeQMoeMLP(config, intermediate_size=config.moe_intermediate_size) for _ in range(self.num_experts)]
        )
...
class KeyeQMoeRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        KeyeQMoeRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
        self.weight = nn.Parameter(torch.ones(hidden_size))
        self.variance_epsilon = eps
...
class KeyeQMoeDecoderLayer(nn.Module):
    def __init__(self, config: KeyeQMoeConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = KeyeQMoeAttention(config, layer_idx)

        if (layer_idx not in config.mlp_only_layers) and (
            config.num_experts > 0 and (layer_idx + 1) % config.decoder_sparse_step == 0
        ):
            self.mlp = KeyeQMoeSparseMoeBlock(config)
        else:
            self.mlp = KeyeQMoeMLP(config, intermediate_size=config.intermediate_size)

        self.input_layernorm = KeyeQMoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = KeyeQMoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class KeyeQMoeRotaryEmbedding(nn.Module):
    def __init__(self, config: KeyeQMoeConfig, device=None):
        super().__init__()
...
class KeyeQMoePreTrainedModel(PreTrainedModel):
    config_class = KeyeQMoeConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["KeyeQMoeDecoderLayer"]
    _skip_keys_device_placement = ["past_key_values"]
    _supports_flash_attn_2 = True
    _supports_sdpa = True
    _supports_flex_attn = True
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if module.padding_idx is not None:
                module.weight.data[module.padding_idx].zero_()
        elif isinstance(module, KeyeQMoeRMSNorm):
            module.weight.data.fill_(1.0)


class KeyeQMoeModel(KeyeQMoePreTrainedModel):
    def __init__(self, config: KeyeQMoeConfig):
        super().__init__(config)
        ...
        self.layers = nn.ModuleList(
            [KeyeQMoeDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = KeyeQMoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = KeyeQMoeRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
    ...
    def _update_causal_mask(
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of KeyeQMoe. Make sure to "
        ...
        if self.config._attn_implementation == "flex_attention":
            if isinstance(attention_mask, torch.Tensor):
                raise Exception(
                    "Flex Attention does not support attention_mask. Please use `attention_mask` instead of `attn_mask`."
                )
...
class KeyeQMoeForCausalLM(KeyeQMoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = KeyeQMoeModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs
        ...
        ```python
        >>> from transformers import AutoTokenizer, KeyeQMoeForCausalLM

        >>> model = KeyeQMoeForCausalLM.from_pretrained("Qwen/Qwen3-MoE-15B-A2B")
...
class KeyeQMoeForConditionalGeneration(KeyeQMoePreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    config_class = KeyeQMoeConfig
...
class Projector(nn.Module):
    ...
    def __init__(self, text_config: KeyeQMoeConfig,vision_config: KeyeQMoeVisionConfig):
        super().__init__()
