...
# https://www.docvqa.org/datasets/infographicvqa and
...
# raise Exception("Answer #" + str(i) + " not valid (invalid question ID.
# Expected:" + str(gtObject['questionId']) + "Found:" +
# detObject['questionId'] + ")")
      ...
      if isinstance(detObject['answer'], list):
        raise Exception(
            "Question " +
            str(
                gtObject['questionId']) +
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
  ...
  methodMetrics = {
      'score': 0 if len(
          gtJson['data']) == 0 else totalScore / (
          len(
              gtJson['data']) - len(question_ids_to_exclude))}
  ...
  if show_scores_per_answer_type:
    for a_type, ref in answer_types.items():
      answer_types_scores[ref] = 0 if len(
          gtJson['data']) == 0 else answerTypeTotalScore[a_type] / (answerTypeNumQuestions[a_type])
  ...
  if show_answer_types:
    ...
    for a_type in answer_types.values():
      print(
          "\t{:12s} {:2.4f}".format(
              a_type,
              results['scores_by_types']['answer_types'][a_type]))
    ...
    for e_type in evidence_types.values():
      print(
          "\t{:12s} {:2.4f}".format(
              e_type,
              results['scores_by_types']['evidence_types'][e_type]))
    ...
    for r_type in reasoning_requirements.values():
      print(
          "\t{:12s} {:2.4f}".format(
              r_type,
              results['scores_by_types']['operation_types'][r_type]))
