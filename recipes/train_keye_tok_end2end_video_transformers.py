...
# Modified for muse framework - KeyeTokenizerEnd2EndVideoTransformers training
#
# This script uses the transformers-style modeling code from end2end repo
# for cross-repository compatibility testing.
#
...
"""
KeyeTokenizerEnd2EndVideoTransformers Training Script.

This script implements training for the KeyeForConditionalGeneration model
using transformers-style modeling code from end2end repo.

Usage:
    python recipes/train_keye_tok_end2end_video_transformers.py \
        --model-dir /path/to/model \
        --output-dir /path/to/output \
        --dataset-config examples/keye_tokenizer_end2end_video_transformers/config.json \
        ...
"""
...
# Import transformers-style model from end2end repo style modeling
from muse.models.keye_tokenizer_end2end_video_transformers.modeling import (
    KeyeForConditionalGeneration,
    KeyeDecoderLayer,
    SiglipEncoderLayer
)
from muse.models.keye_tokenizer_end2end_video_transformers.configuration_keye import (
    KeyeConfig as TransformersKeyeConfig
)
...
def train():
    ...
    # Determine training mode and load config
    # For transformers-style models, we use HuggingFace config format
    if args.model_dir:
        # Continue pretrain mode: load config from model_dir/config.json (HuggingFace format)
        hf_config_path = Path(args.model_dir) / "config.json"
        if hf_config_path.exists():
            print_rank_0(f"Loading HuggingFace config from: {hf_config_path}")
            model_config = TransformersKeyeConfig.from_pretrained(args.model_dir)
        else:
            # Fallback to muse config if HuggingFace config not found
            model_config_path = Path(args.model_dir) / "muse_config.json"
            if not model_config_path.exists():
                raise FileNotFoundError(
                    f"Config file not found: {hf_config_path} or {model_config_path}. "
    ...
    # Set attention implementation for transformers-style config
    if args.use_flash_attention_2:
        if hasattr(model_config, '_attn_implementation'):
            model_config._attn_implementation = "flash_attention_2"
        if hasattr(model_config, 'qwen_config'):
    ...
    # For transformers-style model, we use KeyeForConditionalGeneration
    model_class_name = "KeyeForConditionalGeneration"
    ...
    # Use transformers-style KeyeForConditionalGeneration directly
    print_rank_0(f"Using transformers-style model: {model_class_name}")
    model_cls = KeyeForConditionalGeneration
