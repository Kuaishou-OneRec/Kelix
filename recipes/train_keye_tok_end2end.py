...
def train():
    ...
    while True:
        ...
        with contextlib.ExitStack() as ctx:
            ...
            token_metrics_tensor = torch.tensor(
                [num_tokens, num_samples, num_valid_tokens], 
                dtype=torch.float32, device=torch.cuda.current_device()
            )
            ticker.tick("token_metrics_init")
            
            dist.all_reduce(token_metrics_tensor, op=dist.ReduceOp.SUM, group=get_data_parallel_group())
            ticker.tick("token_metrics_reduce")
            
            # 获取汇总后的值 (乘以 context_parallel_size 得到真实全局值)
            num_tokens, num_samples, num_valid_tokens = (
                token_metrics_tensor[0].item() * get_context_parallel_world_size(),
                token_metrics_tensor[1].item() * get_context_parallel_world_size(),
                token_metrics_tensor[2].item() * get_context_parallel_world_size()
            )
            ...
            # ================================================ Forward pass ================================================
            # Note: Do NOT pass labels to model - loss is computed externally (same as video)
            ...
            if local_loss_mask is not None:
                ...
                total_valid_tokens_count = (local_mask_shifted > 0).sum().float()
                
                dist.all_reduce(total_loss_sum, op=dist.ReduceOp.SUM)
                dist.all_reduce(total_valid_tokens_count, op=dist.ReduceOp.SUM)
            ...
            # Record metrics (使用全局平均值记录到 TensorBoard，与 video 对齐)
            ...
            with Timer("bwd"):
                ...
                if scheduler.is_gradient_accumulation_boundary():
                    ...
                    # 在 lr_scheduler.step() 之前获取 learning rate，与 video 对齐
                    # 使用 get_last_lr() 获取当前 learning rate
                    learning_rate = lr_scheduler.get_last_lr()[0]
                    metrics.append("learning_rate", learning_rate)
                    
                    # Get vision learning rate - 使用 get_lr() 与 video 对齐
