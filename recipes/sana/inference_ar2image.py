...
def _compute_output_dir(args) -> str:
    """Compute output dir from model/dcp and benchname.

    约定：
    - 如果提供了 dcp_ckpt_dir + dcp_tag：输出到 `${dcp_ckpt_dir}/${dcp_tag}/inference/{benchname}/outputs`
    - 否则如果提供了 model_dir：输出到 `${model_dir}/inference/{benchname}/outputs`
    - 否则回退到当前目录 `./inference/{benchname}/outputs`

    注意：`--output-dir` 仍保留参数，但视为 deprecated。
    """

    if getattr(args, "dcp_ckpt_dir", None) and getattr(args, "dcp_tag", None):
        return os.path.join(args.dcp_ckpt_dir, args.dcp_tag, "inference", args.benchname, "outputs")

    if getattr(args, "model_dir", None):
        return os.path.join(args.model_dir, "inference", args.benchname, "outputs")

    return os.path.join(".", "inference", args.benchname, "outputs")


def main():
    args = parse_args()

    computed_output_dir = _compute_output_dir(args)

    # NOTE: output-dir 不再推荐外部传入；保留兼容，但只提示（不报错）
    # NOTE: args.output_dir 仍然保留参数（兼容老脚本），但已经 deprecated。
    # - 我们不再允许通过它改变 output_dir（以免 twobenches 下目录混乱）
    # - 仅当用户显式传入且与 computed 不一致时给出 warning
    if hasattr(args, "output_dir") and args.output_dir and args.output_dir != computed_output_dir:
        warnings.warn(
            f"--output-dir is deprecated and will be ignored. "
            f"Got output_dir={args.output_dir}, computed_output_dir={computed_output_dir}",
            category=UserWarning,
        )

    # 统一后续逻辑使用 computed_output_dir
    args.output_dir = computed_output_dir
