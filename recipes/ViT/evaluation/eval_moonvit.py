...
def get_image_embedding(model, batch):
    processor = model.processor
    moon_processor = model.MoonVIT_processor
    images = batch["images"]
    inputs = moon_processor(images=images, return_tensors="pt")

    pixel_values = to_cuda(inputs["pixel_values"]).to(torch.bfloat16)
    image_grid_hws = to_cuda(inputs["image_grid_hws"])

    moonvit = model.MoonVIT
    vision_embeds = moonvit(pixel_values, image_grid_hws)
    vision_embeds = model.mlp(vision_embeds)
