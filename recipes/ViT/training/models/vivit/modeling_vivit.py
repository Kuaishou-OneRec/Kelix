...
class VivitTubeletEmbeddings(nn.Module):
    ...
    Modified to handle variable-length videos where the number of frames is a multiple of the configured num_frames.
    """
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        # Check if the video is longer than the configured num_frames
        if num_frames > self.num_frames and num_frames % self.num_frames == 0:
            # Process longer videos in chunks
            chunks = num_frames // self.num_frames
            all_embeddings = []
            
            for i in range(chunks):
                start_idx = i * self.num_frames
                end_idx = (i + 1) * self.num_frames
                chunk_values = pixel_values[:, :, start_idx:end_idx, :, :]
                
                # Process this chunk
                chunk_embed = self.projection(chunk_values)
                chunk_embed = chunk_embed.flatten(2).transpose(1, 2)
                all_embeddings.append(chunk_embed)
                
            # Concatenate all embeddings along the sequence length dimension
            x = torch.cat(all_embeddings, dim=1)
        else:
            # Original processing for videos with expected number of frames
...
class VivitEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Save the base number of patches for a standard video length
        self.base_num_patches = self.patch_embeddings.num_patches
    ...
    def extend_position_embeddings(self, embeddings: torch.Tensor) -> torch.Tensor:
        #TODO: 现在只是机械的重复，看看能否处理成有长序列前后关系的。
        """
        Extends the position embeddings to handle longer videos.
        For videos with multiple segments (length = N * num_frames), we repeat the position embeddings.
        """
        num_patches = embeddings.shape[1] - 1  # Exclude CLS token
        num_positions = self.position_embeddings.shape[1] - 1  # Base number of positions
        
        # If the number of patches matches, or if it's smaller, just return the original embeddings
        if num_patches <= num_positions:
            return self.position_embeddings
        
        # Calculate how many repetitions we need
        num_segments = (num_patches + num_positions - 1) // num_positions
        
        # Get the class token embedding and patch embeddings separately
        class_pos_embed = self.position_embeddings[:, :1]  # CLS token embedding
        patch_pos_embed = self.position_embeddings[:, 1:]  # Patch embeddings
        
        # Repeat the patch embeddings for each segment
        repeated_pos_embed = patch_pos_embed.repeat(1, num_segments, 1)
        
        # Trim to the exact size needed
        if repeated_pos_embed.shape[1] > num_patches:
            repeated_pos_embed = repeated_pos_embed[:, :num_patches, :]
            
        # Combine with the class token embedding
        extended_pos_embed = torch.cat((class_pos_embed, repeated_pos_embed), dim=1)
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        # Check if we need to handle a longer video (multiple of original num_frames)
        if num_frames % self.config.num_frames == 0 and num_frames > self.config.num_frames:
            # Extend the position embeddings for longer videos
            pos_embeddings = self.extend_position_embeddings(embeddings)
...
VIVIT_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_frames, num_channels, height, width)`):
            ...
            This model can handle variable-length videos where the number of frames is a multiple of the 
            original `config.num_frames` (e.g., 2x, 3x, etc. of the default number of frames).
