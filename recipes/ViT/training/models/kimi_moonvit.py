...
class KimiViT(nn.Module):
    ...
    def calcul_loss(self, text_embeds, image_embeds):
        if self.is_dist:
            ...
            m1_diag1 = -logits_per_text.new_ones(logits_per_text.shape) + 2 * eye

            lik = (m1_diag1 * logits_per_text).to(device)

            loglik = torch.sigmoid(lik)
            loglik = torch.log(loglik)

            # loglik = torch.log(torch.sigmoid(lik))
            # loglik = torch.nn.functional.logsigmoid(lik)
            # if self.ctx.rank == 0:
            #     print(loglik.shape, logits_per_text)
    ...
    def forward(self, images, texts, package=None):
        ...
        outputs = self.siglip.get_text_features(**inputs)
        text_embeds = outputs
        ...
        # Contrastive loss
        Contrastive_loss = self.calcul_loss(text_embeds, image_features)
        ...
        return Context(
            ...
            AR_loss=AR_loss,
            loss=loss,
            total_image_num_tokens=sum([emb.shape[0] for emb in image_features_for_AR]),
            total_text_num_tokens=np.prod(inputs["input_ids"].shape).item(),
