        ...
        with contextlib.ExitStack() as ctx:
            ...
            if vq_indices is not None:
                for cb_idx, indices_tensor in enumerate(vq_indices):
                    ...
                    print(f"[MUSE DEBUG] rank={current_rank}, codebook={cb_idx}, image_indices unique={unique_count}/{total_count}, indices_shape={indices_tensor.shape}")
            ...
            if video_vq_indices_debug is not None:
                for cb_idx, indices_tensor in enumerate(video_vq_indices_debug):
                    unique_count = torch.unique(indices_tensor).numel()
                    total_count = indices_tensor.numel()
                    print(f"[MUSE DEBUG] rank={current_rank}, codebook={cb_idx}, video_indices unique={unique_count}/{total_count}, indices_shape={indices_tensor.shape}")
            # ============ END DEBUG ============
