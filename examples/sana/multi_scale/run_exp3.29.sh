...
# 324的token，产出1024的图像，做position_scale
OUTPUT_DIR=/mmu_mllm_hdd_2/zhouyang12/output/MuseV2/sana_v2/multi_scale/exp3.29
...
nohup mpirun --allow-run-as-root \
        ...
        bash -c "python3 recipes/sana/train_auto_encoder.py \
                ...
                --resolution-budgets "512:32,768:16,1024:16" \
                --learning-rate 2e-5 \
                --min-lr 1e-7 \
                --weight-decay 0.0 \
                --image-size 1024 \
                --model-config-overrides use_position_scale=True \
                --beta1 0.9 \
                --multi-scale \
                --beta2 0.999 \
                --batch-size 16 \
