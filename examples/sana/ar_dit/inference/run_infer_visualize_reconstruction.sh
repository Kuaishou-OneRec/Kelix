...
KEYE_AR_DIR="/mmu_mllm_hdd_2/zhouyang12/output/Keye/vqar_11.9/v2_stage3_1e-4/./step23000/global_step23000/muse_converted"
...
RESULTS_DIR="./results"
NUM_IMAGES=8
DEVICE="cpu"           # set to "cuda" if running on GPU
DTYPE="float32"        # float32 for CPU runs; bfloat16/float16 for GPU runs
...
python inference/keye_ar_sana/infer_visualize_reconstruction.py \
  ...
  --seed ${SEED} \
  --results-dir "${RESULTS_DIR}" ${INIT_DIST_FLAG}

echo "Done. Visualization results (if any) will be in: ${OUTPUT_DIR}/visualization/step_0"