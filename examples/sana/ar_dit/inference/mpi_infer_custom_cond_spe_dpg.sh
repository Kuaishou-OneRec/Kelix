...
# 支持通过外部 env 覆盖 OUTPUT_DIR，从而支持 twobenches：
# - auto_infer_eval.py 会为不同 bench 设置不同 OUTPUT_DIR
# - 这里不要再强制写死 GenEval
...
echo "[twobenches] OUTPUT_DIR resolved to: $OUTPUT_DIR"
