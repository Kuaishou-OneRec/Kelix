...
For each release directory, the script:
    ...
    2. Creates a temporary directory that mirrors the original model directory,
       ...
       and the local `assets/` folder is copied to `assert/` to match the
       `<img src="assert/...">` paths used inside README.md.
...
def _copy_non_safetensors(source_dir: str, temp_dir: str) -> None:
    ...
    The local `assets/` subdirectory (if present) is copied to `assert/` inside
    `temp_dir` to match the `<img src="assert/...">` paths used in README.md.
    """
    for entry in os.listdir(source_dir):
        ...
        if os.path.isdir(src_path) and entry == "assets":
            dst_path = os.path.join(temp_dir, "assert")
            logger.info("Copying assets/ -> assert/")
...
def upload_one(
    ...
    try:
        # 1. Copy non-safetensors files (config.json, README.md, assets -> assert, ...).
