...
OUTPUT_DIR=/mmu_mllm_hdd_2/maosiyang/output/keye_tok_e2e_purevideo_single/MuseV2_transformers/video/stage1_debug
...
# 注意修改实验内容备注
# 使用 end2end 的 transformers-style modeling 代码，通过 msy_master_2 训练框架训练
comment="keye_tokenizer_end2end_video_transformers_stage1"
...
nohup mpirun --allow-run-as-root \
        ...
        -x https_proxy=\
        with_nccl_local_env \
        bash -c "bash numa_runner.sh python3 recipes/train_keye_tok_end2end_video_transformers.py \
                --model-dir $MODEL_DIR \
                --output-dir $OUTPUT_DIR \
                --dataset-config examples/keye_tokenizer_end2end_video_transformers/run_exp1.6.8_stage1.json \
                --lr 2e-4 \
                --vision_lr 2e-5 \
                --min_lr 1e-7 \
                --weight-decay 0.1 \
