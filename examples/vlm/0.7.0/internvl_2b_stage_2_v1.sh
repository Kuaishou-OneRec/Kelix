...
OUTPUT_DIR=/llm_reco_ssd/luoxinchen/output3/RecoVLM-Base/0.7.0/2b/stage_2_v1
rm -rf $OUTPUT_DIR
mkdir -p $OUTPUT_DIR
...
nohup mpirun --allow-run-as-root \
        ...
        python3 recipes/train_fsdp.py --model_dir $MODEL_DIR \
                ...
                --dataset_config examples/vlm/configs/0.7.0/2b_v0_6_0_internvl_stage2.1.json  \
                --max_length 21000 \
                --learning_rate 4e-5 \
		...
		--monitor_image_tokens \
                ...
                --commit_id $git_hash \
		--logging_per_step 10 \
		--resume_from /llm_reco_ssd/luoxinchen/output3/RecoVLM-Base/0.7.0/2b/stage1_5_v4/step2000 \
