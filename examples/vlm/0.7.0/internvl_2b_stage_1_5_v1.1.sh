...
OUTPUT_DIR=/llm_reco_ssd/luoxinchen/output3/RecoVLM-Base/0.7.0/2b/stage2_v1.1
rm -rf $OUTPUT_DIR
mkdir -p $OUTPUT_DIR
...
# 注意修改实验内容备注
comment="run internvl 2b 0.7.0 stage1.5 by lzx, use stage2-0.6.0 data"
...
nohup mpirun --allow-run-as-root \
		...
		--monitor_image_tokens \
                ...
                --commit_id $git_hash \
		--logging_per_step 10 \
		--resume_from /llm_reco_ssd/luoxinchen/output3/RecoVLM-Base/0.7.0/2b/stage1_v0/step12000 \
