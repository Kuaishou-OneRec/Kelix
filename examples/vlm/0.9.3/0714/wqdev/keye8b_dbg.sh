...
sed 's/=1/=8/g' /etc/mpi/hostfile | head -32 > /etc/mpi/hostfile_seq
...
OUTPUT_DIR=/llm_reco/nasen/wq-outputs/recovlm/keye-${timestamp}
OUTPUT_DIR=/llm_reco/nasen/wq-outputs/recovlm/keye-32gpu-${timestamp}
mkdir -p $OUTPUT_DIR
...
git_hash=12345 #$(git rev-parse --short HEAD)
...
nohup mpirun --allow-run-as-root \
        ...
        bash -c "bash numa_runner.sh python3 recipes/train_fsdp.py --model_dir $MODEL_DIR \
                ...
                --dataset_config /llm_reco/nasen/keye_stage3_video_0730_sp1_4k.json \
