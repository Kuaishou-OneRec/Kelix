...
OUTPUT_DIR=/llm_reco_ssd/luoxinchen/output3/RecoVLM-Base/0.7.1/2b/stage_3_v4
rm -rf $OUTPUT_DIR
mkdir -p $OUTPUT_DIR
...
nohup mpirun --allow-run-as-root \
        ...
        python3 recipes/train_fsdp.py --model_dir $MODEL_DIR \
                ...
                --dataset_config examples/vlm/configs/0.7.0/2b_v0_7_0_internvl_stage3_bl_v3.json  \
                --max_length 21000 \
                --learning_rate 5e-5 \
		...
		--monitor_image_tokens \
                ...
                --commit_id $git_hash \
		--logging_per_step 10 \
		--resume_from /llm_reco_ssd/luoxinchen/output3/RecoVLM-Base/0.7.1/2b/Stage2_0.1.0/step55000 \
		--resume_from_tag global_step55000 \
                --kml_id $KML_ID \
