...
sed 's/=1/=8/g' /etc/mpi/hostfile | head -12 > /etc/mpi/hostfile_seq
...
OUTPUT_DIR=/mmu_mllm_hdd_2/zhouyang12/output/Keye/0.8.7/stage1_0.3.7/v701
...
nohup mpirun --allow-run-as-root \
	...
	with_nccl_local_env \
        bash -c "bash numa_runner.sh python3 recipes/train_fsdp.py --model_dir $MODEL_DIR \
                ...
                --dataset_config examples/vlm/0.8.7/keye_8B_stage3_v7.json \
