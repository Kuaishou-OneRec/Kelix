...
OUTPUT_DIR=/llm_reco/lingzhixin/output/internvl-2b/balance_llm_flops
OUTPUT_DIR=/llm_reco/lingzhixin/output/qwen3navit/intern3b/0.2.0/2B256/
rm -rf $OUTPUT_DIR
mkdir -p $OUTPUT_DIR
...
nohup mpirun --allow-run-as-root \
	...
	bash -c "unset http_proxy && unset https_proxy && bash numa_runner.sh python3 recipes/train_fsdp.py --model_dir $MODEL_DIR \
                ...
                --dataset_config examples/vlm/configs/2b_internvl_stage2_cut2.json \
                --max_length 21000 \
                --learning_rate 4e-7 \
