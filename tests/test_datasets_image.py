...
class TestResolutionBudgetConfig:
    ...
    def test_init_with_budgets(self):
        """Test initialization with budgets list"""
...
class TestParseResolutionBudgets:
    ...
    def test_parse_multiple_resolutions(self):
        """Test parsing multiple resolutions"""
        config = parse_resolution_budgets("512:32,768:16,1024:8")
        
        assert len(config.budgets) == 3
        assert config.budgets[0].size == 512
        assert config.budgets[1].size == 768
        assert config.budgets[2].size == 1024
