...
class KeyeForConditionalGeneration(Qwen3PreTrainedModel, GenerationMixin):
        ...
        **kwargs
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                if n_image_tokens != n_image_features:
                    raise ValueError("maosiyang::image features and image tokens do not match",image_grid_thw,n_image_tokens, n_image_features)
            ...
            if pixel_values_videos is not None:
                ...
                video_grid_thw_split = split_thw(video_grid_thw.squeeze(0))
                siglip_position_ids = list()
                video_grid_hws = list()
                sample_indices = list()
                cu_seqlens = [0]

                for idx, thw in enumerate(video_grid_thw_split):
                    thw_tuple = tuple(thw.detach().cpu().numpy().tolist())
                    numel = np.prod(thw_tuple)
        ...
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            ...
            if (
                ...
                position_3d_ids, rope_deltas = self.get_rope_index_slowfast(
                    input_ids,
                    image_grid_thw,
                    video_grid_thw,
                    fast_video_grid_thw,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Qwen3-5-VL position_ids are prepareed with rope_deltas in forward
...
class Projector(nn.Module):
    ...
    def __init__(self, text_config: KeyeConfig, vision_config: KeyeVisionConfig):
        ...
        self.temporal_merge_mode = getattr(self.text_config, "temporal_merge_mode", "avg")
        self.temporal_merge_position = getattr(
            self.text_config,
            "temporal_merge_position",
            getattr(self.text_config, "temport_merge_position", "before"),
        )
        ...
        if self.temporal_merge_position == "after" and self.temporal_merge_mode not in ["avg", "delta", "resampler", "deltanet"]:
            raise ValueError(
                "temporal_merge_mode must be one of ['avg', 'delta', 'resampler', 'deltanet'] when temporal_merge_position='after'"
            )
        ...
        self.output_hidden_size = self.text_config.hidden_size
        ...
        self.temporal_delta_net_after = None
        ...
        if self.temporal_merge_position == "after" and self.temporal_merge_mode == "delta":
            self.temporal_delta_norm_after = torch.nn.LayerNorm(self.output_hidden_size, eps=1e-05)
            self.temporal_delta_rnn_after = nn.GRU(
                input_size=self.output_hidden_size,
                hidden_size=self.output_hidden_size,
                num_layers=1,
                batch_first=True,
            )

        self.temporal_resampler = None
        if self.temporal_merge_position == "after" and self.temporal_merge_mode == "resampler":
            num_queries = getattr(self.text_config, "query_num", 256)
            num_heads = getattr(
                self.text_config,
                "temporal_resampler_num_heads",
                max(1, self.output_hidden_size // 128),
            )
            adaptive = getattr(self.text_config, "temporal_resampler_adaptive", False)
            max_hw = getattr(self.text_config, "temporal_resampler_max_hw", (70, 70))
            batch_infer = getattr(self.text_config, "temporal_resampler_batch_infer", False)
            self.temporal_resampler = Resampler(
                num_queries=num_queries,
                embed_dim=self.output_hidden_size,
                num_heads=num_heads,
                kv_dim=self.output_hidden_size,
                adaptive=adaptive,
                max_size=max_hw,
    ...
    def _temporal_reduce_after(self, x: torch.Tensor, spatial_h: int, spatial_w: int) -> torch.Tensor:
        ...
        if self.temporal_merge_mode == "delta":
            ...
            return h_n[-1]
        if self.temporal_merge_mode == "resampler":
            t = x.size(0)
            frames_list = [
                x[idx].reshape(spatial_h * spatial_w, self.output_hidden_size)
                for idx in range(t)
            ]
            tgt_sizes = torch.tensor(
                [[spatial_h, spatial_w]] * t,
                dtype=torch.long,
                device=x.device,
            )
            temporal_ids = [list(range(t))]
            if not temporal_ids:
                return x.mean(dim=0)
            resampled = self.temporal_resampler(frames_list, tgt_sizes=tgt_sizes, temporal_ids=temporal_ids)
            return resampled.permute(1, 0, 2).reshape(-1, self.output_hidden_size)
        raise ValueError(f"Unsupported temporal_merge_mode {self.temporal_merge_mode} for after merge.")
    ...
    def _process_sequence(self, image_feature: torch.Tensor, image_grid: Tuple[int, int, int]) -> torch.Tensor:
        ...
        if t == 0:
            return image_feature.new_zeros((0, self.output_hidden_size))
...
class Resampler(nn.Module):
    ...
    def __init__(
        ...
        self.query = nn.Parameter(torch.randn(self.num_queries, embed_dim)*0.02)
        print("maosiyang_init:",self.query)
        ...
        # self.ln_q =  torch.nn.LayerNorm(embed_dim, eps=1e-05)
        # self.ln_kv =  torch.nn.LayerNorm(embed_dim, eps=1e-05)

        # self.ln_post =  torch.nn.LayerNorm(embed_dim, eps=1e-05)
    ...
    def forward(self, x,tgt_sizes=None, temporal_ids=None):
        # 输入 x: List[Tensor], 每个元素为单帧特征，形状 [Li, KvDim]
        # 目标：按 temporal_ids 指定的分组将各帧拼接为可变长序列，
        # 利用 flash_attn_varlen_func 对重复的查询与分组后的 K/V 做变长注意力计算。

        bs = len(x)  # 帧数

        # print('maosiyangdebug::',self.query)

        device = x[0].device
        dtype = x[0].dtype

        # If built under meta device, rebuild cached buffers on the current device
        if hasattr(self, "pos_embed") and isinstance(self.pos_embed, torch.Tensor) \
                and self.pos_embed.device.type == "meta":
            self._set_2d_pos_cache(self.max_size, device)
        if hasattr(self, "temporal_pos_embed") and isinstance(self.temporal_pos_embed, torch.Tensor) \
                and self.temporal_pos_embed.device.type == "meta":
            self._set_temporal_pos_cache(self.max_temporal_size, device)

        # 根据每帧的网格大小生成 2D 位置编码，并根据 temporal_ids（若提供）准备时间编码
        ...
        if temporal_ids is not None:
            ...
            max_temporal_size = max(temporal_ids_flatten) + 1 if len(temporal_ids_flatten) > 0 else 0
            if max_temporal_size > -1:
                temporal_pos_emb = True
            if max_temporal_size > self.max_temporal_size:
                self._adjust_temporal_pos_cache(max_temporal_size, device)

        # 预处理每帧：投影到 embed_dim，LayerNorm，并构造 K/V 与 2D+时间位置编码
        per_frame_len: List[int] = []
        per_frame_k: List[torch.Tensor] = []
        per_frame_v: List[torch.Tensor] = []

        for i in range(bs):
            tgt_h, tgt_w = tgt_sizes[i]
            # [Li, D]
            kv_i = self.kv_proj(x[i].to(device=device, dtype=dtype))
            kv_i = self.ln_kv(kv_i)
            v_i = kv_i  # V 不加位置

            # 2D 位置编码 [Li, D]
            pos2d_i = self.pos_embed[:tgt_h, :tgt_w, :].reshape((tgt_h * tgt_w, -1)).to(device=device, dtype=dtype)

            # 时间位置编码 [D]，需要广播到 [Li, D]
            if temporal_pos_emb:
                if temporal_ids_flatten[i] == -1:
                    tpe = torch.zeros(self.embed_dim, dtype=dtype, device=device)
                else:
                    tpe = self.temporal_pos_embed[temporal_ids_flatten[i]].to(device=device, dtype=dtype)
            else:
                tpe = None

            k_i = kv_i + pos2d_i if tpe is None else kv_i + pos2d_i + tpe.unsqueeze(0)

            per_frame_len.append(k_i.shape[0])
            per_frame_k.append(k_i)
            per_frame_v.append(v_i)

        # 构造分组后的可变长序列 K/V，以及对应的 cu_seqlens
        assert temporal_ids is not None and len(temporal_ids) > 0
        num_groups = len(temporal_ids)

        group_k_list: List[torch.Tensor] = []
        group_v_list: List[torch.Tensor] = []
        k_lens: List[int] = []

        for grp in temporal_ids:
            if len(grp) == 0:
                group_k_list.append(torch.empty(0, self.embed_dim, device=device, dtype=dtype))
                group_v_list.append(torch.empty(0, self.embed_dim, device=device, dtype=dtype))
                k_lens.append(0)
                continue
            k_cat = torch.cat([per_frame_k[idx] for idx in grp], dim=0)
            v_cat = torch.cat([per_frame_v[idx] for idx in grp], dim=0)
            group_k_list.append(k_cat)
            group_v_list.append(v_cat)
            k_lens.append(k_cat.shape[0])

        # 查询重复（每组共享同一组 learnable queries）
        q_base = self.ln_q(self.query).to(dtype=dtype, device=device)  # [Q, D]
        Q = q_base.shape[0]
        q_cat = torch.cat([q_base for _ in range(num_groups)], dim=0)  # [Q*num_groups, D]
        ...
        max_seqlen_q = Q
        max_seqlen_k = max(k_lens) if len(k_lens) > 0 else 0

        k_cat_all = torch.cat(group_k_list, dim=0) if len(group_k_list) > 0 else torch.empty(0, self.embed_dim, device=device, dtype=dtype)
        v_cat_all = torch.cat(group_v_list, dim=0) if len(group_v_list) > 0 else torch.empty(0, self.embed_dim, device=device, dtype=dtype)

        # 使用 MultiheadAttention 的投影权重来生成 Q/K/V 供 Flash-Attn 使用
        embed_dim = self.embed_dim
        num_heads = self.num_heads
        head_dim = embed_dim // num_heads
        scale = 1.0 / math.sqrt(head_dim)

        q_proj = self.q_proj(q_cat).to(dtype=dtype, device=device).contiguous()
        k_proj = self.k_proj(k_cat_all).to(dtype=dtype, device=device).contiguous()
        v_proj = self.v_proj(v_cat_all).to(dtype=dtype, device=device).contiguous()

        # 变形为 [total_len, num_heads, head_dim]
        q_proj = q_proj.view(-1, num_heads, head_dim)
        k_proj = k_proj.view(-1, num_heads, head_dim)
        v_proj = v_proj.view(-1, num_heads, head_dim)
        ...
        attn_output = attn_output.reshape(-1, embed_dim)  # [Q*num_groups, D]
        attn_output = self.out_proj(attn_output).to(dtype=dtype, device=device)

        # 组织输出为 [num_groups, Q, D]，对齐 resampler.forward 的后处理
        x = attn_output.view(num_groups, Q, embed_dim)
        x = self.ln_post(x).to(dtype=dtype, device=device)
        x = x @ self.proj

        # 返回 [Q, num_groups, D] 以匹配上层调用的 permute(1,0,2)
