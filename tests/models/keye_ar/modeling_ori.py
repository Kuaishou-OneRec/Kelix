...
from .token_decoder import PureDecoderTransformer
# from configuration_keye import KeyeConfig, KeyeVisionConfig, KeyeImageTokenizerConfig
# from vq import VectorQuantizer
...
class SiglipEncoder(nn.Module):
    ...
    def forward(
        ...
        # print_input_info({"hidden_states": hidden_states, "encoder_states": encoder_states}, "SiglipEncoder forward output")
...
class KeyeAttention(nn.Module):
    ...
    def __init__(self, config: KeyeConfig, layer_idx: Optional[int] = None):
        ...
        # print(config)
        # # 1024 16 64 8
        # print(config.hidden_size, config.num_attention_heads, getattr(self, 'head_dim', None), self.num_key_value_heads)
        # print(self.k_proj)
        # print(self.v_proj)
        # print("\n" * 5)
...
def find_high_error_positions(
    ...
    """
    找到 ref_emb 和 inputs_embeds 误差大于阈值的位置，返回对应位置的 input_ids 及其索引
    
    Args:
        ref_emb: 参考嵌入向量，shape = [1, 23912, 1024]
        inputs_embeds: 输入嵌入向量，shape = [1, 23912, 1024]
        input_ids: 输入ID序列，shape = [1, 23912]
        threshold: 误差阈值，大于该值的位置会被筛选出来（默认1.0）
        norm_type: 误差计算的范数类型（1=L1范数，2=L2范数，默认L2）
    
    Returns:
        dict: 包含以下key的结果字典：
            - "error_indices": 高误差位置的第二维索引（0-based），shape = [M,] where M为高误差位置数
            - "corresponding_input_ids": 高误差位置对应的input_ids，shape = [M,]
            - "corresponding_errors": 高误差位置的具体误差值，shape = [M,]
    """
    # # -------------------------- 1. 输入格式校验 --------------------------
    # assert ref_emb.shape == (1, 23912, 1024), \
    #     f"ref_emb 形状错误！期望 (1, 23912, 1024)，实际 {ref_emb.shape}"
    # assert inputs_embeds.shape == (1, 23912, 1024), \
    #     f"inputs_embeds 形状错误！期望 (1, 23912, 1024)，实际 {inputs_embeds.shape}"
    # assert input_ids.shape == (1, 23912), \
    #     f"input_ids 形状错误！期望 (1, 23912)，实际 {input_ids.shape}"
    # assert norm_type in (1, 2), "norm_type 仅支持 1（L1范数）或 2（L2范数）"
    
    # 确保所有张量在同一设备上
    device = ref_emb.device
    assert inputs_embeds.device == device and input_ids.device == device, \
        "所有输入张量必须在同一设备上（CPU/GPU）"
    
    # -------------------------- 2. 计算逐位置误差 --------------------------
    # 计算两个嵌入的差值（shape: [1, 23912, 1024]）
    emb_diff = ref_emb - inputs_embeds
    
    # 按最后一维（特征维）计算范数，得到逐位置误差（shape: [1, 23912]）
    # 范数类型由 norm_type 指定（L1/L2）
    per_position_error = torch.norm(emb_diff, p=norm_type, dim=-1)
    
    # 去除 batch 维度（shape: [23912,]），简化后续计算
    per_position_error = per_position_error.squeeze(0)
    input_ids_squeezed = input_ids.squeeze(0)  # 同样去除batch维：[23912,]
    
    # -------------------------- 3. 筛选高误差位置 --------------------------
    # 找到误差大于阈值的位置索引（0-based，对应第二维）
    high_error_indices = torch.where(per_position_error > threshold)[0]
    
    # 提取对应位置的 input_ids 和误差值
    corresponding_input_ids = input_ids_squeezed[high_error_indices]
    corresponding_errors = per_position_error[high_error_indices]
    
    # -------------------------- 4. 返回结果 --------------------------
    result = {
        "error_indices": high_error_indices,  # 高误差位置的第二维索引
        "corresponding_input_ids": corresponding_input_ids,  # 对应input_ids
        "corresponding_errors": corresponding_errors  # 对应误差值（方便验证）
    }

    # 打印结果
    print(f"找到 {len(result['error_indices'])} 个高误差位置：")
    for idx, input_id, error in zip(
        result["error_indices"],
        result["corresponding_input_ids"],
        result["corresponding_errors"]
    ):
        print(f"index: {idx:4d} | Input ID:{input_id:6d} error:{error:.4f}, ref_emb:{ref_emb[0][idx][:10]}, pred_emb:{inputs_embeds[0][idx][:10]}")
...
class KeyeDecoderLayer(nn.Module):
    def __init__(self, config: KeyeConfig, layer_idx: int):
        ...
        if config._attn_implementation != "flash_attention_2":
            logger.warning_once(
                f"Flash Attention 2 is enabled but not implemented for `{config._attn_implementation}`; "
                "unexpected results may be encountered."
            )
...
class KeyeCausalLMOutputWithPast(ModelOutput):
    ...
    aligned_indices: Optional[torch.LongTensor] = None  # 改为 Optional
    logits: Optional[torch.FloatTensor] = None  # 改为 Optional
    past_key_values: Optional[List[torch.FloatTensor]] = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    rope_deltas: Optional[torch.LongTensor] = None
    reconstruction_loss: Optional[torch.LongTensor] = None
    extended_tokens: Optional[torch.LongTensor] = None
...
class KeyeImageTokenizer(PreTrainedModel):
    ...
    def __init__(self, config: KeyeImageTokenizerConfig, 
        ...
        self.pre_llm_align = getattr(config, 'pre_llm_align', False) # 是否把原来的vit projector出来的东西先对齐到llm的维度

        print(f"param: self.n_q_tokens={self.n_q_tokens}, self.split_dim={self.split_dim}, self.split_voc={self.split_voc}, self.add_voc_reducer={self.add_voc_reducer}, self.pre_llm_align={self.pre_llm_align}")
        ...
        self.encoder = nn.Linear(llm_align_size if self.pre_llm_align else config.llm_hidden_size, config.embedding_dim if self.split_dim else self.n_q_tokens * config.embedding_dim)

        # TODO: fix hard code
        self.quantizer = nn.ModuleList([
            VectorQuantizer(
                num_embeddings=config.codebook_size,
                embedding_dim=config.embedding_dim // self.n_q_tokens if self.split_dim else config.embedding_dim,
                ...
                split_voc_index=split_voc_index
            ) for split_voc_index in range(self.n_q_tokens)])
        self.post_init()
    ...
    def forward(self,
        ...
        image_embeds = self.pre_llm_aligner(image_embeds) # .detach()
...
class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: KeyeConfig):
        ...
        self.pre_embedding_size = getattr(config, "pre_embedding_size", None)
        self.pre_embedding_tokens = getattr(config, "pre_embedding_tokens", None)

        if getattr(config, "new_table", False) and self.pre_embedding_size is None:
            self.embed_tokens = nn.Embedding(config.vocab_size + config.vision_config.codebook_size, config.hidden_size, self.padding_idx)
            print("Using new table", self.embed_tokens)
        ...
        if self.pre_embedding_size is not None:
            ...
            self.pre_embedding_linear = nn.Linear(self.pre_embedding_size, config.hidden_size)
    ...
    def forward(
        ...
        **kwargs
        ...
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError(f"You must specify exactly one of input_ids or inputs_embeds, input_ids={input_ids}, inputs_embeds={inputs_embeds}")
...
def expand_with_image_tokens(
    ...
    """
    拓展input_ids矩阵，将image_token_id对应的行替换为image_indices和eos_token
    
    参数说明：
        image_indices: 图像索引矩阵，维度为 (im_len, n_q_tokens)
        input_ids: 原始输入ID矩阵，维度为 (len, 1)
        eos_token: EOS标记的整数ID
    ...
    assert image_indices.dim() == 2, \
        f"image_indices必须是 2D 矩阵，当前为 {image_indices.shape}"
    
    len_seq = input_ids.size(0)  # 序列长度
    output_dim = 1 + n_q_tokens  # 输出矩阵的列数
    
    # 1. 初始化输出矩阵，所有位置先填充padded_token
    expanded_ids = torch.full(
        size=(len_seq, output_dim),
        fill_value=padded_token,
        dtype=input_ids.dtype,
        device=input_ids.device
    )
    
    # 2. 找到input_ids中等于image_token_id的行索引
    image_token_mask = (input_ids.squeeze(1) == image_token_id)  # (len,)
    image_token_indices = torch.nonzero(image_token_mask, as_tuple=True)[0]  # 满足条件的行索引
    
    # 校验：image_indices的行数必须等于image_token的数量
    assert image_indices.size(0) == len(image_token_indices), \
        f"image_indices的行数 ({image_indices.size(0)}) 必须等于input_ids中image_token_id的数量 ({len(image_token_indices)})"
    
    # 3. 处理非image_token的行
    # 第一列填充input_ids的值，第二列填充eos_token，其余保持padded_token
    ...
    if output_dim > 1:  # 确保至少有第二列
        expanded_ids[non_image_mask, 1] = eos_token  # 第二列：eos_token
    
    # 4. 处理image_token的行
    # 前n_q_tokens列填充image_indices，最后一列填充eos_token
    if len(image_token_indices) > 0:  # 只有存在image_token时才处理
        # 前n_q_tokens列：填充对应位置的image_indices
        expanded_ids[image_token_indices, :n_q_tokens] = image_indices
        # 最后一列：填充eos_token
        expanded_ids[image_token_indices, -1] = eos_token
...
class KeyeForConditionalGeneration(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.visual_tokenizer = KeyeImageTokenizer(config.vision_config, llm_align_size=original_hidden_size)
        self.quant_projector = nn.ModuleList([nn.Linear(config.vision_config.embedding_dim // config.vision_config.n_q_tokens \
                                                        if config.vision_config.split_dim else config.vision_config.embedding_dim, original_hidden_size, bias=False) for _ in range(self.visual_tokenizer.n_q_tokens)])
        
        # def quant_projector_v2():
        #     in_dim = config.vision_config.embedding_dim // config.vision_config.n_q_tokens if config.vision_config.split_dim else config.vision_config.embedding_dim
        ...
        # self.quant_projector_v2 = nn.ModuleList([quant_projector_v2() for _ in range(self.visual_tokenizer.n_q_tokens)])
        self.model = Qwen3Model(config)
        # self.reconstruction_projector = nn.Linear(config.hidden_size, config.vision_config.llm_hidden_size, bias=False)
        self.vocab_size = config.vocab_size
        self.config = config
        ...
        print(f"self.ar_mode={self.ar_mode}")
        ...
        self.token_head = PureDecoderTransformer(
            ...
            nhead = getattr(self.config, 'token_head_nhead', 16), # 不能小于16会报错
            num_layers = 1,
            dim_feedforward = getattr(self.config, 'token_head_intermediate_dim', self.config.hidden_size * 2) ,
            # embed_tokens = self.model.embed_tokens,
            # lm_head=nn.Identity(),
            vocab_size=lm_head_size,
            use_flash_attn=getattr(self.config, 'token_head_use_flash_attn', False),
            input_dim=self.config.hidden_size,
            reduce=self.config.hidden_size != getattr(self.config, 'token_head_dim', self.config.hidden_size)
        )
        self.output_one_token = getattr(config, 'output_one_token', False)
        self.detachim = getattr(config, 'detachim', False)
        
        self.chunked_loss_computer = getattr(config, "chunked_loss_computer", False)
        assert self.pool in ['avg', 'sum'], f"pool must be 'avg' or 'sum', but got {self.pool}"
        # Initialize weights and apply final processing

        self.lm_head = nn.Linear(config.hidden_size, lm_head_size, bias=False) # to cheat transformer check
        self.post_init()
        self.lm_head = nn.Linear(config.hidden_size, lm_head_size, bias=False)

        print(f"self.pool={self.pool}, self.amplifier={self.amplifier}, self.output_one_token={self.output_one_token}, self.chunked_loss_computer={self.chunked_loss_computer}, self.lm_head={self.lm_head}, self.detachim={self.detachim}")
    ...
    def infer_id_embs(self, extended_tokens, group_size=None):
        ...
        if self.model.pre_embedding_size is not None:
            ...
            visual_embeds_final = stage1_embeds # .sum(dim=2)
        else:
            stage2_embeds = self.model.embed_tokens(safe_visual_indices)
            visual_embeds_final = stage2_embeds# .sum(dim=2)

        mask_final = is_visual_group.unsqueeze(-1).expand_as(text_embeds)

        # mask_final=torch.Size([1, 3640, 4096]), visual_embeds_final=torch.Size([1, 3640, 8, 4096]), text_embeds=torch.Size([1, 3640, 4096])
    ...
    def forward(
        ...
        **kwargs
        ...
        # input_image_ids = None # lzx debug
        x0 = None
        ref_image_mask = None
        x1 = None
        image_embeds = None
        if inputs_embeds is None and input_image_ids is None:
            ...
            if pixel_values is not None:
                ...
                train_dict["aligned_indices"] = torch.stack([x_i for x_i in vq_out['indices']], 0).T # l * n_q_tokens
                train_dict["aligned_indices"] = self.vocab_size + train_dict["aligned_indices"] + torch.arange(self.config.vision_config.n_q_tokens).\
                ...
                if getattr(self.config, 'new_table', False):
                    ...
                    #     print("Using new table, so detach the inputs_embeds for image tokens.")
                    # inputs_embeds = self.model.embed_tokens(input_ids, input_image_ids=train_dict["aligned_indices"].to(input_ids))
                    input_image_ids = train_dict["aligned_indices"].to(input_ids)
                    inputs_embeds = None
                else:
                    input_ids = None
                    
            if attention_mask is not None:
                attention_mask = attention_mask.to(inputs_embeds.device if inputs_embeds is not None else input_ids.device)
        ...
        ###############################################################################################################################################
        ###############################################################################################################################################
        # print("extended_tokens", train_dict["aligned_indices"], train_dict["aligned_indices"].shape, input_ids.shape)
        # if train_dict["aligned_indices"].numel() <= 1:
        ...
        if input_image_ids.numel() <= 1:
            input_image_ids = torch.zeros(0, self.config.vision_config.n_q_tokens + 1).to(input_ids)
        extended_tokens = expand_with_image_tokens(input_image_ids.detach(), input_ids.flatten()[:,None], self.config.q_eos_token, -100, self.config.image_token_id, self.config.vision_config.n_q_tokens)
        ...
        if self.output_one_token:
            ...
            # if os.environ.get("OF_DEBUG") and 0:
            #     # run_8b_vis_stage3_transformer_tokenhead512_allavg_train_textemb_genpipe_of_v2
            #     from recovlm.utils.ds_utils import print_input_info
            #     infered = self.infer_id_embs(extended_tokens)
            #     print(f"inferedinfered={infered.shape}, token_inputs_embeds={token_inputs_embeds.shape}")
            #     print_input_info({
            #         "hidden_states": hidden_states,
            #         "token_inputs_embeds_next": token_inputs_embeds_next,
            #         "token_inputs_embeds": token_inputs_embeds,
            #         "extended_tokens": extended_tokens,
            #         "extended_infered": infered,
            #     }, "keye_token_embedding", save_path=os.environ.get("OF_DEBUG") + ".pth")
        ...
        loss = None
        assert labels is None, "Labels should be None."

        if not return_dict:
            output = (logits,) + outputs[1:]
            return (loss,) + output if loss is not None else output
        
        res = KeyeCausalLMOutputWithPast(
            **train_dict,
            logits=logits,
        ...
        res.extended_tokens = extended_tokens
        res.reconstruction_loss_dict = reconstruction_loss_dict
        return res
