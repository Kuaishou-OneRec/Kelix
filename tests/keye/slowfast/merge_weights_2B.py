...
def main():
  ...
  fast_siglip_pt = {}
  with safe_open("/llm_reco_ssd/zhouyang12/models/siglip-base-patch16-224/model.safetensors", framework="pt", device="cpu") as f:
          for key in f.keys():
              if key.startswith("vision_model"):
                # cjx_key = key.replace("vision_model", "visual_fast")
                fast_siglip_pt["visual_fast." + key] = f.get_tensor(key)
...
#   print('lalallalalallal')
  ...
  pt2 = {}
  for i in range(1, 3):
      with safe_open("/llm_reco_ssd/zhouyang12/models/Keye-1.7B-demo_hf_vit_rope/model.safetensors", framework="pt", device="cpu") as f:
          for key in f.keys():
              pt2[key] = f.get_tensor(key)
  print('lalallalalallal')
...
#     pt2[key] = pt1[key]
  ...
  outputdir = "/llm_reco_ssd/zhouyang12/models/Keye-1.7B_vitrope_slowfast_0612"
