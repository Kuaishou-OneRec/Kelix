...
from .unified_token_decoder import PureDecoderTransformer
# from configuration_keye import KeyeConfig, KeyeVisionConfig, KeyeImageTokenizerConfig
# from vq import VectorQuantizer
        ...
        **kwargs
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # print("ttttttt", train_dict["aligned_indices"].shape, train_dict["aligned_indices"])
                # if torch.distributed.get_rank() == 0:
                #     print(f"aligned_indices={train_dict['aligned_indices'].shape}")

                if 0:
                    # 2b 模型可以开这个debug
                    image_embeds = sum([quant_projector_i(x_i) for x_i, quant_projector_i in zip(vq_out['z_q'], self.quant_projector) ]) # * self.amplifier
                    if self.ar_mode == 'ar':
