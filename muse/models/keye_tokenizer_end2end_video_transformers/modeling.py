...
try:
    ...
    # Use muse.training.parallel instead of recovlm.training.parallel
    from muse.training import parallel as mpu
    from muse.training.parallel import (
        get_context_parallel_group as get_sequence_parallel_group,
        get_context_parallel_world_size as get_sequence_parallel_world_size,
...
class KeyeForConditionalGeneration(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual_tokenizer = KeyeImageTokenizer(config.vision_config, llm_align_size=config.hidden_size)
        self.quant_projector = nn.ModuleList([nn.Linear(config.vision_config.embedding_dim // config.vision_config.n_q_tokens \
                                                        if config.vision_config.split_dim else config.vision_config.embedding_dim, config.hidden_size, bias=False) for _ in range(self.visual_tokenizer.n_q_tokens)])
        ...
        print(f"self.pool={self.pool}, self.amplifier={self.amplifier}")
    ...
    # ==================== Methods required by muse training framework ====================
    def get_layers_to_shard(self):
        """Return a list of layers that should be sharded by FSDP.
        
        This combines layers from both the LLM (self.model.layers) and 
        the visual tokenizer (self.visual_tokenizer.visual.vision_model.encoder.layers).
        
        Note: The path matches end2end's shard_model logic:
        layers = list(model.visual_tokenizer.visual.vision_model.encoder.layers) + list(model.model.layers)
        """
        model_layers = list(self.model.layers)
        visual_layers = list(self.visual_tokenizer.visual.vision_model.encoder.layers)
    ...
    def get_checkpointable_module_classes(self):
        ...
        return {KeyeDecoderLayer, SiglipEncoderLayer}
    
    def get_initializer(self, name: str):
        """Return initializer function for a given parameter name.
        
        Args:
            name: The parameter name (e.g., 'model.layers.0.self_attn.q_proj.weight')
        
        Returns:
            A callable that initializes the parameter tensor.
        """
        # Use lecun_normal_ as default, similar to other muse models
    ...
    def get_optimizer_grouped_parameters(
        ...
        if hasattr(self, 'visual_tokenizer') and hasattr(self.visual_tokenizer, 'visual'):
            if hasattr(self.visual_tokenizer.visual, 'vision_model'):
                num_vision_layers = len(self.visual_tokenizer.visual.vision_model.encoder.layers)
        ...
        if vision_lr_layer_decay < 1.0 and num_vision_layers > 0:
            ...
            # No layer decay - single group for vision
    ...
    # ==================== End of muse training framework methods ====================
        ...
        **kwargs
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # print("projected indices", torch.stack([x_i for x_i in vq_out['indices']], 0).T)
                stacked = torch.stack([quant_projector_i(x_i) for x_i, quant_projector_i in zip(vq_out['z_q'], self.quant_projector) ], 0)
                # print(stacked.shape)
                stacked = stacked.transpose(0,1)
                # print(f"stackeddd")
                # print(stacked)
                # image_embeds = sum([self.quant_projector(x) for x in vq_out['z_q']], 0) / len(vq_out['z_q'])
                image_embeds = sum([quant_projector_i(x_i) for x_i, quant_projector_i in zip(vq_out['z_q'], self.quant_projector) ]) * self.amplifier
                # image_embeds = sum([quant_projector_i(x_i) for x_i, quant_projector_i in zip(vq_out['z_q'], self.quant_projector_v2)], 0)

                if self.pool == 'avg':
                    image_embeds = image_embeds / len(self.quant_projector)
                
                if image_embeds.shape == vq_out['x'].shape:
                    train_dict["reconstruction_loss"] = F.mse_loss(image_embeds, vq_out['x'].detach())
                    reconstruction_loss_dict = {
                        "reconstruction_loss_diff_mu": (image_embeds.detach() - vq_out['x'].detach()).mean(),
