...
class PromptLoader:
  ...
  def __init__(self, library_dir: Optional[str] = None):
    if not library_dir:
      ...
      library_dir = os.path.join(cur_dir, "prompts")
...
class TemplateLoader:
  ...
  def load(self, path_or_template: Optional[str] = None) -> str:
    ...
    if not path_or_template.endswith(".jinja"):
      path_or_template += ".jinja"
    if not os.path.exists(path_or_template):
      path_or_template = os.path.join(self.library_dir, path_or_template)
    if os.path.exists(path_or_template):
      with open(path_or_template, encoding="utf-8") as f:
        return f.read()
    return raw
