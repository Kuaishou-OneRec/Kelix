...
def fetch_image(ele: dict[str, str | Image.Image], size_factor: int = IMAGE_FACTOR, is_video = False) -> Image.Image:
    ...
    if "force_assistant_image_size" in ele:
        print(f"force_assistant_image_size={ele['force_assistant_image_size']}")
        # if np.random.rand() < 0.0001: print(f"force_assistant_image_size={ele['force_assistant_image_size']}")
        # image = image.resize(ele["force_assistant_image_size"], ele["force_assistant_image_size"])
        image = resize_and_center_crop(image, ele["force_assistant_image_size"], ele["force_assistant_image_size"])
    else:
        image = image.resize((resized_width, resized_height))
    # if np.random.rand() < 0.001: print(f"imageimageimage={image}")
    return image
